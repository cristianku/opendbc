...
# [artiv probe] - START
def _artiv_test_controller(platform=CAR.PSA_PEUGEOT_3008):
...
def test_artiv_waits_again_after_moving_without_bursting_or_restarting():
  controller, cs = _artiv_test_controller()
  for frame, stationary in ((0, True), (500, False), (1000, False), (1999, True)):
...
def test_artiv_tester_present_is_not_enabled_for_other_psa_platforms():
  for platform in CAR:
    if platform != CAR.PSA_PEUGEOT_3008:
      ...
      for frame in (200, 1000, 1500, 10000):
